...
- 菜单栏图标、图标大小和文字字号可配置。
- 百分比可显示为剩余量或使用量。两档警示阈值（默认剩余 30%、10%）让额度依次变橙、变红，菜单栏文字、菜单窗口与 Touch Bar 同步。
- 进度条上有一条速度参考线，标出按时间平均消耗此刻应到的位置；消耗明显偏快的额度还会显示倍率标记。
- 默认跟随系统语言，并可在设置中切换简中、繁中、英文、日文、韩文和西班牙文。
- 支持登录时自动启动；Codex 每分钟刷新，Claude 最多每 5 分钟请求一次，使用电池时加倍，显示器休眠时暂停。
...
设置窗口左侧是服务、显示、菜单栏、Touch Bar 和通用五个分栏。

“显示”分栏可以把所有百分比在“剩余量”和“使用量”之间切换（进度条随之改变含义），开关速度参考线，并设置两档警示阈值。阈值上方的色带会随滑杆实时显示三个区间分别是什么颜色。默认剩余 30% 变橙、10% 变红。
...
该接口按账号限流，额度与 Claude Code 本身以及其他使用同一接口的工具共享。收到 HTTP 429 后应用会先等待 5 分钟，之后每次连续 429 翻倍，最长 1 小时；此期间自动刷新会跳过 Claude，但菜单里的“刷新”按钮每分钟仍可手动尝试一次。切换 Claude 账号会清除该退避状态，因为限流额度是按账号计算的。
