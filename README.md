...
- 菜单栏图标、图标大小和文字字号可配置。
- 百分比可显示为剩余量或使用量，并可自定义橙色、红色警示阈值，菜单与 Touch Bar 同步变色。
...
设置页的“显示”分组可以把所有百分比在“剩余量”和“使用量”之间切换（进度条随之改变含义），并设置橙色与红色警示的阈值：达到阈值时，菜单窗口与 Touch Bar 的百分比和进度条会变色。默认剩余 25% 变橙、10% 变红。
